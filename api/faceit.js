...
export default async function handler(request, response) {
  ...
  const nickname = request.query.nick;
  const viewTemplate = request.query.view; // если есть &view=
  ...
  try {
    ...
    if (matchesResponse.ok) {
      ...
      last30Stats.matches_count = lastMatches.length;
      let totalKills = 0, totalRounds = 0, totalADR = 0;
      lastMatches.forEach(match => {
        totalKills += match.kills;
        totalRounds += match.rounds;
        totalADR += match.adr;
    ...
    const result = {
      ...
      api: {
        ...
        elo: playerData.games?.cs2?.faceit_elo || 0,
        last_5_matches: last5MatchesTrend,
        last_30_stats: last30Stats
      }
    };

    // Если есть &view=, возвращаем текст
    if (viewTemplate) {
      const textOutput = viewTemplate.replace(/\{(\w+)\}/g, (_, key) => {
        // Поддержка плейсхолдеров из result.api.last_30_stats и result.api
